...
if tmpFileCheck:
    # 从文件读取所有股票基本数据
    print('从文件读取所有股票基本数据')
    all_stock = pd.read_csv(tmp_file_all_stock, dtype={'code':np.str_,'totalAssets':np.str_}, index_col=0).sort_values(by='code')
    ...
    all_stock.to_csv(tmp_file_all_stock)
...
timeToMarket,上市日期
"""
result = all_stock.to_sql(table_stock, engine, if_exists = 'replace', index=False)
print(result)
